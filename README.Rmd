...
<Aim>This paper provides a review of the scientific literature on the relationship between road lighting and cycling uptake and identifies existing gaps in research and knowledge.
<Method>We used a hybrid systematic and snowballing method.
TBC studies were reviewed. Subjective and objective studies were found.
<Results>The literature suggests that road lighting has a strong positive effect on cycling levels during the night. The impact is higher among women.
<Gaps>More research on the effect of road lighting specifically on cycling is needed, including in developing countries, in countries with different latitudes to most Western countries, and on the different impact that road lighting has on cyclists of different ages and social groups. 
<Conclusion>
...
From the included studies, we extracted information regarding: author(s) and year, type of method, method, measurements, location of study, and results.

# Results

## Trends of research

<Subject area>The scientific literature was found to be broadly spread across several disciplines, including engineering, environmental science, social science, and public health.

<Study type>Most of the studies were written in the last decade (Figure 1).

```{r fig2, echo=FALSE, fig.cap="Figure 2: number of papers reviewed by type and year", message=FALSE, warning=FALSE}
library(kableExtra)
library(tidyverse)
...
df = papers %>%
  filter(full_text == 1) %>% 
  select(authors, type, method, cycling, lighting, location, result)
...
- More research in countries with different latitudes to most Western countries.
...
<Strength>This is the first review on the relationship between road lighting and cycling uptake.

<Limitations>We could have included the term darkness in the query string?
We could have searched on additional electronic databases. 
