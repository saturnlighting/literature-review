...
@fotios_lighting_2017 reviewed suggested interventions to enable cycling lighting to contribute to the safety of cyclists in the UK. However, to the best of our knowledge, no academic paper has reviewed the international literature available on the intersection between both terms, cycling and road lighting. 
...
5.  Proper lighting for cyclists (i.e. optimal use of lighting to enhance cyclists reassurance while respecting the environment)
...
df = papers %>% replace(is.na(.), 0) %>%
  group_by(type) %>% 
  summarise("Cycling behaviour and lighting" = sum(behaviour), "Cycling collisions and lighting" = sum(road_safety),  "Cycling, crime and lighting" = sum(crime), "Cycling inqualities in lighting" = sum(inequalities), "Proper lighting for cyclists" = sum(proper_light)) %>% 
  pivot_longer((2:6))
...
I.e.
How lighting attributes such as illuminance, light temperature, uniformity and glare, affect cycling.
