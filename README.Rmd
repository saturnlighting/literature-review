...
This is mostly due to a higher perceived danger of pedestrians and cyclists at night from motorised traffic or some other type of violence.
...
Many variables (individual, social, and environmental) can affect an individual's decision to travel by active modes, including by walking, cycling, and wheeling (meaning by non-powered mobility devices, scooters and other wheeled human powered vehicles).

Darkness is one of the environmental factors that most affect walking and cycling. Less studied has been the influence of darkness on wheeling, although we can assume that darkness is a major barrier to wheeling because people who rely on wheeled mobility are among the most vulnerable in society^[The annual [Disability & Cycling](https://wheelsforwellbeing.org.uk/wp-content/uploads/2022/05/Disability-and-Cycling-Report-of-2021-national-survey-results.pdf) report by the charity Wheels for Wellbeing, for example, highlights the importance of mobility and active modes for disabled people.
...
After screening abstracts and full text, 54 studies were selected.
...
df = papers %>% replace(is.na(.), 0) %>%
  group_by(type) %>% 
  summarise("Active travel behaviour and lighting" = sum(behaviour), "Active travel collisions and lighting" = sum(road_safety),  "Active travel, crime and lighting" = sum(crime), "Inqualities in lighting" = sum(inequalities), "Proper lighting" = sum(proper_light)) %>% 
  pivot_longer((2:6))
...
In a review of forty two quantitative studies that estimated association between neighbourhood build environment and active travel in older adults, @cerin_neighbourhood_2017 observed positive association with availability of street lights.
...
"Fortunately, careful lighting design, soundly-based outdoor lighting standards and new lighting and sensor technology offer the possibility of providing the benefits of light at night while minimizing the impact on the environment" [@boyce_benefits_2019].
...
"lighting distribution changes perception" [@cellucci_lighting_2016]. 
...
The positive results obtained in studies looking at the association between active travel uptake and street lighting show the importance of light conditions as a policy measure to encourage active travel in the dark. Lighting conditions improvements not only may increase active trips in general, but also the inclusivity of these modes, since, as has been demonstrated, street lighting will benefit more certain sectors of society such as women, children, the elderly, and certain disadvantaged groups
